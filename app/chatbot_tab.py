...
def render_chatbot_tab(user_id, api_key=None):
    ...
    st.warning(
        "The assistant sends a summarized version of the synthetic demo "
        "data to Groq. Do not enter real financial or personal information."
    )

    consent = st.checkbox(
        "I understand and want to enable the AI assistant.",
        key=f"{user_id}_llm_consent",
    )

    if not consent:
        st.info("Enable consent to use the AI assistant.")
        return
    ...
    recent_context = (
        df.tail(30).groupby("category")["amount"].agg(["count", "sum", "mean"]).round(2).to_csv()
    )
    ...
    with st.form("chat_form", clear_on_submit=True):
        ...
        with col1:
            user_input = st.text_input(
                "Type your question here:", key=input_key, label_visibility="collapsed"
            )
        with col2:
