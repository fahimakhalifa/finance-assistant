...
def render_overview_tab(df, user_id):
    ...
    with st.expander("Adjust your preferences", expanded=False):
        ...
        with col1:
            ...
            salary_input = st.number_input(
                "Monthly Salary",
                min_value=0,
                value=int(st.session_state[f"{user_id}_monthly_salary"]),
                step=100,
            )
    ...
    fig_growth = px.bar(
        ...
        y="category",
        orientation="h",
        title="Category Spending Growth (%)",
    ...
    if selected_day_df.empty:
        ...
        breakdown = (
            selected_day_df.groupby("category")["amount"]
            .sum()
            .reset_index()
    ...
    expected_spend_by_now = st.session_state[f"{user_id}_monthly_salary"] * (
        month_progress_pct / 100
    )
    pacing_status = ""
